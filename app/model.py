...
class PCBCharacteristics(BaseModel):
    ...
    marking_side: str = Field(
        ...
        description=(
            ...
            "Если указан способ нанесения (шелкография / паяльная маска / медь) — "
            "добавьте его через запятую, например 'TOP, шелкография'."
        )
    )
    ...
    date_code: str = Field(
        default="",
        description=(
            "Требование маркировать дату изготовления (месяц/год, дата-код): "
            "'месяц и год изготовления', 'дату изготовления', 'Manufacturing data', "
            "'date code'. Значения: 'Yes'/'No'. Если указан формат "
            "(напр. 'требуется в формате ХХ (римские) ХХ (арабские)') → Yes. "
            "Если указан способ нанесения (шелкография / маска / медь) — "
            "укажите его, например 'Yes, шелкография'."
        )
    )
